...
SELECT EMP_NAME, SALARY*12 연봉, (SALARY + (SALARY*BONUS))*12 "총수령액(보너스포함)"
        , (SALARY + (SALARY*BONUS))*12 - SALARY*12*0.03 "실수령액" FROM EMPLOYEE;
...
FROM EMPLOYEE WHERE (SALARY + (SALARY*BONUS))*12 - SALARY*12*0.03 >= 50000000;
...
SELECT EMP_NAME, DEPT_CODE, SUBSTR(EMP_NO, 1, 2) || '년 ' || SUBSTR(EMP_NO, 3, 2) || '월 ' || SUBSTR(EMP_NO, 5, 2) || '일' "생년월일"
          , TO_CHAR(SYSDATE, 'YYYY') - TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 2), 'RR'), 'YYYY') "나이(만)"
FROM EMPLOYEE;
...
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6) "주민번호 앞자리", SUBSTR(EMP_NO, 8) "주민번호 뒷자리"
                    , SUBSTR(EMP_NO, 1, 6) + SUBSTR(EMP_NO, 8) "주민번호 앞뒤 합"
FROM EMPLOYEE
WHERE EMP_ID = 201;
-- 24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT SUM((SALARY + (SALARY*NVL(BONUS, 0)))*12) "D5 부서 보너스 포함 연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
