...
public class MemberServiceImpl implements MemberService {
	...
	public int insertMember(Member m) {
		/*
		 * 기존 JDBC 방식 - Connection 객체 생성 - DAO에게 Connection 객체와 전달받은 데이터 전달(요청) - 결과에
		 * 따라(DML) -> 트랜잭션 처리 - Connection 객체 반납(close) - 결과 리턴
		 */
		/* MyBatis */
	...
	public int deleteMember(String userId, String userPwd) {
		...
		/*
		 * // (1) map 형태로 가공처리 후 요청 HashMap hMap = new HashMap(); hMap.put("id",
		 * userId); hMap.put("pwd", userPwd);
		 */
		if (result > 0) {
			sqlSession.commit();
		}
